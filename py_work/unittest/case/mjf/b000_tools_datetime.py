...
class B000_Tools_Datetime_Test(unittest.TestCase):
    ...
    def testDateTimeFormatShort(self):
        ...
        if timestamp != 1563010200:
            PrintTip("[StrDT2TimeStamp]字符串日期20190713173000转换为时间戳1563010200失败，测试结果为" + str(timestamp))
            self.assertFalse(True)
        strDT = tools.TimeStamp2StrDT(timestamp , "%Y%m%d%H%M%S")
        if strDT != strTestDT :
            PrintTip("[TimeStamp2StrDT]时间抽1563010200转换为日期20190713173000失败，测试转换结果为" + strDT)
        ...
        if not(tupleDT[0] == 2019 and tupleDT[1] == 7 and tupleDT[2] == 13 and tupleDT[3] == 17 and tupleDT[4] == 30 and tupleDT[5] == 0 ):
            PrintTip("[StrDT2Tuple]字符串日期2019-07-13 17:30:00转换为年月时分秒失败，year:%d , month:%d , mday:%d , hour:%d , minute:%d , second:%d" % tupleDT)
            self.assertFalse(True)
    def testDateTimeFormatStandard(self):
        strTestDT = "2019-07-13 17:30:00"
        timestamp = tools.StrDT2TimeStamp(strTestDT)
        if timestamp != 1563010200:
            PrintTip("[StrDT2TimeStamp]字符串日期2019-07-13 17:30:00转换为时间戳1563010200失败，测试结果为" + str(timestamp))
            self.assertFalse(True)
        strDT = tools.TimeStamp2StrDT(timestamp)
        if strDT != strTestDT :
            PrintTip("[TimeStamp2StrDT]时间抽1563010200转换为日期2019-07-13 17:30:00失败，测试转换结果为" + strDT)
            self.assertFalse(True)
    ...
    def testToolsSleep(self):
        ...
        if elapsetime - 2 > 0.5:
            PrintTip("[Sleep_ms(2000)]误差有点大，测试值:" +  str(elapsetime))
            self.assertFalse(True)
    def testNowDateTime(self):
        ...
        if elapsetime > 2:
            PrintTip("[NowOffset]获取当前时间是失败，[%s][%d][%d]" % (strNowDT , nowstamp , tmptimestamp))
            self.assertFalse(True)
    def testRun(self):
